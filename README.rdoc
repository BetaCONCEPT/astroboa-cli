...
- OPTIONAL: In MAC OS X is strongly recommended to install 'brew' package manager as well as XCODE and XCODE command line tools in order to be able to compile new packages and ruby native extensions.

- Ruby 1.9.x. If you need to install or upgrade your ruby read the following section on 'how to install ruby'. 

- libxml2 and libxslt.
	astroboa-cli uses nokogiri gem which depends on libxml2 and libxslt.
	On *MAC OS X* you do not need to install anything if you already have installed the latest XCODE 
...
- OPTIONAL: Manually install 'pg' gem if you want to install/setup astroboa to work with postgres database.
	...
	In you are ok with derby db then you do not need to install the 'pg' gem.
